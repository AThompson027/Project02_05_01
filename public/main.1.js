//******THIS IS THE MAIN.JS FILE FOR THE FOLLOWERS PAGE FOR FUNCTIONALITY */


(function() {
    // the selected user id is going to equal nothing at the start and reflect which follower the user has collected
    var selectedUserId;
    var cache = {};

    function startup() {
        //this gets the twitter ids of followers by classname
        var followers = document.getElementsByClassName('follower');
        for (var i = 0; i < followers.length; i++) {
            ...
            followers[i].addEventListener('click', function() {
                for (var j = 0; j < followers.length; j++) {
                    followers[j].className = 'follower';
                }
    ...
    function getNotes(userId, callback) {
        //if the userId is cached then we return and have a callback of the cache of notes from our followers
        if (cache[userId]) {
        ...
        xhttp.open('GET', '/followers/' + encodeURIComponent(userId) + '/notes');
    ...
    function postNewNote(userid, note, callback) {
        ...
        xhttp.open('POST', '/followers/' + encodeURIComponent(userid) + '/notes' );
        ...
        xhttp.open('PUT', '/followers/' + encodeURIComponent(userid) + '/notes/' + encodeURIComponent(note._id), true );
        xhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
        xhttp.send(JSON.stringify(note));
    }
    ...
    xhttp.open('DELETE', '/followers/' + encodeURIComponent(userid) + '/notes/' + encodeURIComponent(note._id), true );
    xhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
    xhttp.send(JSON.stringify(note));
}
