//THIS IS THE INDEX FOR DISPLAYING FOLLOWERS
//WHEN YOU CLICK THE "Click to see followers" LINK IT WILL RUN THIS PAGE!
...
var storage = require("./storage.1.js");
storage.connect();
...
// This will show all the followers data
...
app.get('/login', function(req, res) {
    res.render('login');
    if (storage.connected) {
        console.log("We are deleting followers collection on login!");
        storage.deletefollowers();
    }
});
...
app.get('/logout', function(req, res) {
    ...
    if (storage.connected) {
        console.log("We are deleting followers collection on logout!");
        storage.deletefollowers();
    }
    res.redirect('/login');
});
...
//THIS IS THE CODE THAT WHEN YOU CLICK THE "Click to see all friends!" LINK THAT IT WILL GO BACK TO DISPLAYING THE FRIENDS!
...
app.get('/friends', function(req, res){
    ...
    authenticator.get(url, credentials.access_token, credentials.access_token_secret, function(error, data) {
        ...
        if (storage.connected()) {
            // storage.insertFriends(data);
        }
    });
});

//---------------------------------------------
// This the start of the renderMainPageFromTwitter1 function that is tied to render the friends from twitter
